...
DistanceAngleRegulator::DistanceAngleRegulator(
  ...
  odometry_subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "odom", 10,
      std::bind(&DistanceAngleRegulator::odometry_callback, this, _1));
  twist_publisher_ =
  ...
  parameters_callback_handle_ = this->add_on_set_parameters_callback(
      std::bind(&DistanceAngleRegulator::parameters_callback, this,
                std::placeholders::_1));
  /**********************************************/
  ...
  this->get_parameter("d_term_filter_distance",
                      regulator_distance_.d_term_filter_coefficient);
  ...
  this->get_parameter("d_term_filter_angle",
                      regulator_angle_.d_term_filter_coefficient);
  ...
  process_frame_thread_ =
      std::thread(&DistanceAngleRegulator::process_robot_frame, this);
  ...
  // Disabling Synchronization is important. We want independent control for
  // distance and angle.
...
void DistanceAngleRegulator::odometry_callback(
  ...
  int sign = 1;
  if (std::abs(angle_normalize(odom_robot_angle_ - distance_increment_angle)) >
      0.1) {
    sign = -1;
  }
...
void DistanceAngleRegulator::control_loop() {
  ...
  /* RUCKIG */
  motion_profile_result_ =
      motion_profile_->update(motion_profile_input_, motion_profile_output_);

  if (motion_profile_result_ == ruckig::Result::Working) {
    regulator_distance_.reference = motion_profile_output_.new_position[0];
    regulator_angle_.reference =
        angle_normalize(motion_profile_output_.new_position[1]);
  ...
  if (debug_) {
    RCLCPP_INFO(this->get_logger(), "Robot distance: %lf",
                odom_robot_distance_);
    RCLCPP_INFO(this->get_logger(), "Regulator distance reference: %lf",
    ...
    RCLCPP_INFO(this->get_logger(), "Robot angle deg: %lf",
                odom_robot_angle_ * 180.0 / M_PI);
    RCLCPP_INFO(this->get_logger(), "Robot angle reference deg: %lf",
                regulator_angle_.reference * 180.0 / M_PI);
    RCLCPP_INFO(this->get_logger(), "Regulator angle error deg: %lf\n",
...
DistanceAngleRegulator::parameters_callback(
...
void DistanceAngleRegulator::motion_command() {
  ...
  if (action_running_) {
    RCLCPP_WARN(
        rclcpp::get_logger(""),
        "Tried calling motion command action while other action is running!");
...
void DistanceAngleRegulator::forward(double distance) {
  ...
  motion_profile_input_.target_position[0] =
      motion_profile_input_.current_position[0] + distance;
}
...
void DistanceAngleRegulator::rotate_relative(double angle) {
  ...
  motion_profile_input_.target_position[1] =
      motion_profile_input_.current_position[1] + angle;
}
...
void DistanceAngleRegulator::softstop() {
  ...
  double linear_stop_distance =
      std::pow(robot_velocity_linear, 2) /
      (2.0 * motion_profile_input_.max_acceleration[0]);
  double angular_stop_distance =
      std::pow(robot_velocity_angular, 2) /
      (2.0 * motion_profile_input_.max_acceleration[1]);

  linear_stop_distance =
      std::copysign(linear_stop_distance, robot_velocity_linear);
  angular_stop_distance =
      std::copysign(angular_stop_distance, robot_velocity_angular);
...
bool DistanceAngleRegulator::angle_regulator_finished() {
  return motion_profile_finished() &&
         (std::abs(regulator_angle_.error) < angle_goal_tolerance_);
}
...
void DistanceAngleRegulator::navigate_to_pose() {
  ...
  if (distance_to_goal <= distance_goal_tolerance_ &&
      std::abs(angle_normalize(goal_angle - map_robot_angle_)) <=
          angle_goal_tolerance_) {
    action_running_ = false;
